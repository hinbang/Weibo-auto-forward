...
class WeiBoLogin(object):
    ...
    def add_new(self, content):
        add_url = "https://weibo.com/aj/mblog/add?ajwvr=6&__rnd=1504967427943"
        add_data = {
        'text':content,
        'rank':0,
        'rankid':'',
        ...
        print(content)
        headers={}
        headers ['set-cookie']= self.jsonText.headers['set-cookie']
        headers['Referer'] = 'http://weibo.com/u/'+self.user_uniqueid+'?topnav=1&wvr=5'
        print(self.user_uniqueid)
        resp = self.session.post(add_url,data=add_data,headers=headers)
        jsontext = resp.text
    ...
    def forward(self,mid,content):
        forwardurl = "https://weibo.com/aj/mblog/forward?__rnd=1516557266248"
        data = {
        'pic_src':'',
        'pic_id':'',
        'appkey':'',
        ...
        headers = {}
        headers['set-cookie'] = self.jsonText.headers['set-cookie']
        headers['Referer'] = 'http://weibo.com/u/'+self.user_uniqueid+'?topnav=1&wv=5'
        resp = self.session.post(forwardurl, data, headers=headers)
        jsontext = resp.text
        json_text = json.dumps(jsontext)
        print(json_text)
